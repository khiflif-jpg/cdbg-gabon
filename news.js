/* ===========================
   ...
   Version corrigée pour PFBC + ATIBT
   =========================== */
...
(() => {
  ...
  // ✅ Flux RSS
  const RSS_URL_OVERRIDE_1 = "https://www.cdbg-gabon.com/get-rss-pfbc.php";  // PFBC via proxy
  ...
  const STATIC_ARTICLES = [
    ...
    { lang:"fr", title:"L’économie du bois au Gabon en 2025 : de la coupe au produit fini", description:"Analyse complète de la filière bois gabonaise : exploitation, transformation locale, exportations et durabilité.", img:"article4.avif", link:"article-full4-fr.html", date:"2025-11-04" },
    { lang:"en", title:"Gabon’s Wood Economy in 2025: From Harvest to Finished Products", description:"Comprehensive analysis of Gabon's wood sector: forestry, local processing, exports and sustainability.", img:"article4.avif", link:"article-full4-en.html", date:"2025-11-04" },
    { lang:"fr", title:"Code forestier de la République du Gabon (édition 2025 – CDBG) | Version PDF", description:"Version PDF du Code forestier de la République du Gabon (édition 2025 – CDBG).", img:"code-forestier-pdf.avif", link:"article-full5-fr.html", date:"2025-11-08" },
    { lang:"en", title:"Forest Code of the Republic of Gabon (2025 Edition – CDBG) | PDF Version", description:"PDF version of the Forest Code of the Republic of Gabon (2025 Edition – CDBG).", img:"code-forestier-pdf.avif", link:"article-full5-en.html", date:"2025-11-08" }
  ];
  ...
  const isHomePage = () => /(?:^|\/)(index|en)\.html$/.test(location.pathname.toLowerCase());
  ...
  const createCard = (a) => {
    ...
    wrapper.innerHTML=`
      <a href="${a.link}" class="news-card" ${a._isRSS?'target="_blank" rel="noopener noreferrer"':''}>
        <div class="news-image"><img src="${a.img||''}" alt="${a.title}"></div>
  ...
  const createCardSafe = (a)=>{
    ...
    if(!(a.img && a.img.trim().length>0)) imgBlock?.remove();
    return el;
  };
  ...
  function addStickyPDF(lang){
    ...
    findPreviewContainers().forEach(ctn=>{
      if(!ctn) return;
  ...
  const clearAndInjectMultiple=(containers,items,safe=false)=>containers.forEach(ctn=>clearAndInject(ctn,items,safe));
  ...
  const inject=async()=>{
    ...
    if(rssConfigs.length){
      try{
        ...
        rssItems=allRssArrays.flat();
      } catch(e){
        console.error("Erreur RSS, on continue avec les articles locaux",e);
      }
    }

    const mergedForPreview=[...rssItems,...localsForPage].sort((a,b)=>(a.date<b.date?1:-1));
