...
(() => {
  ...
  const createCard = (a) => {
    ...
    let desc=(a.description||"").trim().slice(0,300);
  ...
  const parseRSS=async(url,tag="PFBC")=>{
    ...
    return items.map(it=>{
      ...
      let desc=(it.getElementsByTagName("content:encoded")[0]?.textContent||it.querySelector("description")?.textContent||"").replace(/<[^>]+>/g,"").trim();
